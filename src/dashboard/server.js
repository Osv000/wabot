...
const { verifyLogin, requireAuth, adminExists } = require('./auth');
...
function createServer() {
  ...
  // ---- Signup (one-time only — see createAdminAccount for the race-safe guard) ----
  app.get('/signup', (req, res) => {
    if (adminExists()) return res.redirect('/login?signupClosed=1');
    res.render('signup', { error: null });
  });

  app.post('/signup', (req, res) => {
    if (adminExists()) return res.status(403).send('Signup is closed — an admin account already exists.');

    const username = (req.body.username || '').trim();
    const { password } = req.body;
    if (!username || !password || password.length < 6) {
      return res.render('signup', { error: 'Choose a username and a password of at least 6 characters.' });
    }

    const hash = bcrypt.hashSync(password, 10);
    const created = db.createAdminAccount(username, hash);
    if (!created) {
      // Someone else's signup landed first (race) — nothing to do but send them to login.
      return res.redirect('/login?signupClosed=1');
    }
  ...
  app.get('/login', (req, res) => {
    ...
    res.render('login', { error: null, signupClosed: req.query.signupClosed === '1' });
  });
  ...
  app.post('/login', (req, res) => {
    ...
    try {
      ...
      return res.render('login', { error: 'Invalid username or password.', signupClosed: false });
    } catch (err) {
      return res.render('login', { error: err.message, signupClosed: false });
    }
  });
